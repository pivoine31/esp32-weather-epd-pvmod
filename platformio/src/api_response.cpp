                                        ...
                                        owm_resp_onecall_t &r)
{
  ...
  JsonDocument filter;
  /* AUTO_TZ */
  // Needs access to timezone_offset for automatic timezone management
  // Through this field, OWM indicates the time offset at the indicated location (lat/lon)
  // relative to UTC time. Name of TZ is in timezone
  filter["lat"]             = true;
  filter["lon"]             = true;
  ...
  //Serial.printf("TZ=%s, OFF=%d\n", r.timezone.c_str(),  r.timezone_offset);
